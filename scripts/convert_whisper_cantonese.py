...
def export_sherpa_onnx(model_id: str, output_dir: str, quantize: bool = True):
    ...
    class AudioEncoderWithCrossKV(nn.Module):
        ...
        def forward(self, mel):
            ...
            # Add positional embeddings with DYNAMIC slicing and dtype conversion
            ...
            for layer in self.layers:
                # Call layer - x must be (batch, seq, hidden) and layer returns tuple
    ...
    # Following sherpa-onnx's approach: use negative indexing for cache updates
    class TextDecoderWithKVCache(nn.Module):
        def __init__(self, decoder, embed_tokens, proj_out, n_text_ctx, n_text_state, n_text_head):
            ...
            # Pre-compute causal mask (like official sherpa-onnx)
            # This avoids dynamic mask creation during inference
        ...
        def forward(
            ...
            # Add positional embeddings with explicit dtype conversion
            ...
            # Use list to collect outputs, then stack (avoids in-place issues)
            out_k_list = []
            out_v_list = []

            for i, layer in enumerate(self.decoder.layers):
                # Get layer's KV cache
                layer_k_cache = in_n_layer_self_k_cache[i]  # (batch, n_text_ctx, d_model)
                ...
                # Update KV cache using concatenation (sherpa-onnx approach)
                # Concat new K/V to cached, then take last n_text_ctx entries
                ...
                # Compute self-attention with pre-computed mask
                # k_cache_updated has shape (batch, n_text_ctx, d_model) after concat+slice
                # We only attend to the first (offset_val + seq_len) positions
        ...
        def _multi_head_attention(self, q, k, v, mask=None):
            """Optimized multi-head attention following sherpa-onnx pattern."""
            batch_size = q.shape[0]
            seq_len_q = q.shape[1]
            seq_len_k = k.shape[1]

            # Reshape: (batch, seq, d_model) -> (batch, seq, n_head, head_dim) -> (batch, n_head, seq, head_dim)
            ...
            # Apply mask if provided (for causal self-attention)
            # Mask shape: attn_weights is (batch, n_head, seq_len_q, seq_len_k)
            # We need mask of shape (seq_len_q, seq_len_k) or broadcastable
            if mask is not None:
                # mask is (n_text_ctx, n_text_ctx), we need the last seq_len_q rows and last seq_len_k cols
                # But since we're using cached K/V, we need to mask appropriately
                # The query attends to keys at positions [0, seq_len_k), and query is at positions [seq_len_k - seq_len_q, seq_len_k)
                # So we need: mask[seq_len_k - seq_len_q : seq_len_k, 0 : seq_len_k]
            ...
            # Reshape back: (batch, n_head, seq, head_dim) -> (batch, seq, d_model)
